...
class YOLO(object):
    ...
    def detect_image(self, image, crop=False, count=False):
        ...
        with torch.no_grad():
            ...
            #   将预测框进行堆叠，然后进行非极大抑制
            # ---------------------------------------------------------#
            results = self.bbox_util.non_max_suppression_np(torch.cat(outputs, 1), self.num_classes, self.input_shape,
    ...
    def get_detection_guidance(self, image, resize=False):
        ...
        if resize:
            resize = transforms.Resize((self.input_shape[0], self.input_shape[1]))
            image = resize(image)
    ...
    def get_map_txt(self, image_id, image, class_names, map_out_path):
        ...
        images = transform(image_data).unsqueeze(0)
        ...
        with torch.no_grad():
            ...
            top_label = results[0][:, 6].clone().detach().to(torch.int32)
            top_conf = results[0][:, 4] * results[0][:, 5]
            top_boxes = results[0][:, :4]
